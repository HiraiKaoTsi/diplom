...
class FunctionalMainWindow(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.ui.tabWidget.tabBar().setCursor(QtCore.Qt.PointingHandCursor)
    ...
    def BaseTriggerForUpdate(input_funct: Callable):
        """
    ...
    def DeleteTriggerForUpdate(input_funct: Callable):
        """
        Триггер (декоратор) для обновления информации после изменений данных в программе, что бы триггер обновил
         значения требуется, что бы метод, который оборачивается в декораторе вернул True
        """
    ...
    def OpenPageFunctionalBook(self, id_book: int):
        ...
        self.ui.label_current_price_book.setText(f"По какой стоимости покупалась книга: {data['price']} руб")
    ...
    def AddNewBook(self):
        ...
        price = self.ui.spinBox_price_add.value()

        if (not name) or (not author) or (not isbn) or (quantity == 0) or (price == 0):
        ...
        status = InsertNewBooks((name, author, isbn, year, quantity, price))
        if status:
